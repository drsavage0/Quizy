...
const QUICK_MATCH_GAME_ID = "global-quick-match-game"; // Predefined ID for quick matches
...
export default function MultiplayerLobbyPage() {
  ...
  const handleCreateGame = () => {
    ...
    router.push(`/multiplayer/score-attack/${newGameId}`);
  };

  const handleJoinGame = (e: React.FormEvent) => {
    e.preventDefault();
    if (gameIdToJoin.trim()) {
      router.push(`/multiplayer/score-attack/${gameIdToJoin.trim()}`);
    }
  };

  const handleQuickMatch = () => {
    router.push(`/multiplayer/score-attack/${QUICK_MATCH_GAME_ID}`);
  };
  ...
  return (
    <div className="container mx-auto py-8 flex flex-col items-center space-y-10">
      <header className="text-center">
        ...
        <h1 className="text-4xl font-bold tracking-tight text-primary">{t('multiplayerLobbyTitle')}</h1>
        <p className="text-xl text-muted-foreground mt-2">{t('multiplayerLobbySubtitle')}</p>
      </header>
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <Card className="shadow-xl">
          <CardHeader>
            ...
            <CardDescription>{t('createNewGameDescription')}</CardDescription>
          </CardHeader>
          <CardFooter>
        ...
        <Card className="shadow-xl">
          <CardHeader>
            ...
            <CardDescription>{t('joinExistingGameDescription')}</CardDescription>
          </CardHeader>
